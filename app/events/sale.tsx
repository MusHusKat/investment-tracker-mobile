...
import { apiPost, eventsApi } from "@/lib/api";
...
export default function SaleEventScreen() {
  const params = useLocalSearchParams<{
    ...
    settlementDate?: string; salePrice?: string; agentFee?: string;
    legalFees?: string; otherCosts?: string; mortgageExit?: string; notes?: string;
  }>();
  ...
  const [settlementDate, setSettlementDate] = useState(params.settlementDate ?? today);
  const [salePrice, setSalePrice] = useState(params.salePrice ?? "");
  const [agentFee, setAgentFee] = useState(params.agentFee ?? "");
  const [legalFees, setLegalFees] = useState(params.legalFees ?? "");
  const [otherCosts, setOtherCosts] = useState(params.otherCosts ?? "");
  const [mortgageExit, setMortgageExit] = useState(params.mortgageExit ?? "");
  const [notes, setNotes] = useState(params.notes ?? "");

  const price = Number(salePrice) || 0;
  const totalCosts =
    (Number(agentFee) || 0) + (Number(legalFees) || 0) +
    (Number(otherCosts) || 0) + (Number(mortgageExit) || 0);
  ...
  const doSave = async () => {
    ...
    try {
      if (isEdit) {
        await eventsApi.sale.update(propertyId, body);
        router.back();
      } else {
        await apiPost(`/api/properties/${propertyId}/events/sale`, body);
  ...
  const onSave = () => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "Record Sale", style: "destructive", onPress: doSave },
      ]
    );
  };
  ...
  return (
    <>
      <Stack.Screen options={{
        title: isEdit ? "Edit Sale" : "Record Sale",
        headerShown: true,
      ...
      <SafeAreaView className="flex-1 bg-background" edges={["bottom"]}>
        <ScrollView contentContainerClassName="px-4 py-4" keyboardShouldPersistTaps="handled">
          ...
          <FormField label="Settlement Date" value={settlementDate} onChangeText={setSettlementDate}
            placeholder="YYYY-MM-DD" hint="Format: YYYY-MM-DD" />
          <FormField label="Sale Price ($)" value={salePrice} onChangeText={setSalePrice}
            keyboardType="decimal-pad" placeholder="e.g. 820000" />
          ...
          <FormField label="Agent Commission ($)" value={agentFee} onChangeText={setAgentFee}
            keyboardType="decimal-pad" placeholder="e.g. 16400" hint="Typically 1.5–2.5% of sale price" />
          <FormField label="Legal / Conveyancing Fees ($)" value={legalFees} onChangeText={setLegalFees}
            keyboardType="decimal-pad" placeholder="e.g. 1500" />
          <FormField label="Other Costs ($, optional)" value={otherCosts} onChangeText={setOtherCosts}
            keyboardType="decimal-pad" placeholder="e.g. staging, repairs" />
          <FormField label="Mortgage Exit / Break Fee ($, optional)" value={mortgageExit}
            onChangeText={setMortgageExit} keyboardType="decimal-pad" placeholder="e.g. 0" />
          ...
          <TouchableOpacity
            className={`rounded-2xl py-4 items-center mt-2 ${saving ? "bg-surface" : isEdit ? "bg-primary" : "bg-negative"}`}
            onPress={onSave} disabled={saving}
          >
            {saving ? <ActivityIndicator color={isEdit ? "#6366f1" : "#ef4444"} /> : (
