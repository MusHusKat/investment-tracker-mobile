...
export default function ValuationEventScreen() {
  const params = useLocalSearchParams<{
    ...
    date?: string; value?: string; source?: string; notes?: string;
  }>();
  ...
  const [source, setSource] = useState<Source>((params.source as Source) ?? "SELF");
  ...
  const onSave = async () => {
    ...
    try {
      if (isEdit) {
        await eventsApi.valuation.update(propertyId, params.eventId!, {
    ...
    <>
      ...
      <SafeAreaView className="flex-1 bg-background" edges={["bottom"]}>
        <ScrollView contentContainerClassName="px-4 py-4" keyboardShouldPersistTaps="handled">
          <FormField label="Valuation Date" value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" hint="Format: YYYY-MM-DD" />
          <FormField label="Estimated Value ($)" value={value} onChangeText={setValue} keyboardType="decimal-pad" placeholder="e.g. 750000" />
          ...
          />
          <FormField label="Notes (optional)" value={notes} onChangeText={setNotes} multiline placeholder="e.g. Agent appraisal during lease renewal" />
          ...
          >
            {saving ? <ActivityIndicator color="#6366f1" /> : (
              <Text className="text-white font-semibold text-base">
                {isEdit ? "Save Changes" : "Save Valuation"}
              </Text>
            )}
          </TouchableOpacity>
        </ScrollView>
